...
- CI avoids `pull_request_target`; the standard `pull_request` checks reference no secrets. Separate `@claude` issue/comment/review automation uses `CLAUDE_CODE_OAUTH_TOKEN` with read-only repository permissions plus OIDC token issuance. Provider-backed integration secrets run only on pushes to `main`, the scheduled workflow, or manual dispatch.
