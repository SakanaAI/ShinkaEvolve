...
def test_program_database_init_without_openai_key(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        db = RepoDatabase(config=DatabaseConfig(db_path=str(db_path), num_islands=1))
        try:
            db.add(_program("p0"))
...
def test_async_db_add_without_openai_key_when_embeddings_disabled(monkeypatch):
    ...
    async def _run():
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            async_db = AsyncRepoDatabase(sync_db=sync_db)
            try:
                await async_db.add_repo_async(_program("async-p0"))
...
def test_async_db_add_forwards_verbose_flag(monkeypatch):
    ...
    monkeypatch.setattr(RepoDatabase, "add", tracking_add)
    ...
    async def _run():
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            async_db = AsyncRepoDatabase(sync_db=sync_db)
            try:
                await async_db.add_repo_async(_program("async-p0"), verbose=True)
            finally:
                await async_db.close_async()
                sync_db.close()
    ...
    async def _run():
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                await async_db.add_repo_async(first)
                await async_db.add_repo_async(second)

                assert sync_db.get("async-p0") is not None
                assert sync_db.get("async-p1") is None
                assert sync_db._count_repos_in_db() == 1
                assert sync_db.has_repo_with_source_job_id("job-123") is True
            finally:
                await async_db.close_async()
                sync_db.close()
    ...
    async def _run():
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                assert await async_db.has_repo_with_source_job_id_async("job-123")
            finally:
                await async_db.close_async()
                sync_db.close()
    ...
    async def _run():
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                await async_db.add_repo_async(program)

                recovered = await async_db.get_repo_by_source_job_id_async("job-123")
    ...
    async def _run():
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                await async_db.add_repo_async(duplicate)

                assert sync_db.get("async-p1") is None
                assert sync_db._count_repos_in_db() == 0
            finally:
                await async_db.close_async()
                sync_db.close()
...
def test_async_db_uses_fresh_writer_database_per_add(monkeypatch):
    ...
    async def _run():
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            async_db = AsyncRepoDatabase(sync_db=sync_db)
            original_init = RepoDatabase.__init__
            writer_db_ids = []
            ...
            monkeypatch.setattr(RepoDatabase, "__init__", tracking_init)
            try:
                await async_db.add_repo_async(_program("async-p0"))
                await async_db.add_repo_async(_program("async-p1"))
...
def test_async_db_can_run_multiple_writes_concurrently(monkeypatch):
    ...
    async def _run():
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            async_db = AsyncRepoDatabase(sync_db=sync_db, max_workers=2)
            try:
                await asyncio.gather(
                    async_db.add_repo_async(_program("async-p0")),
                    async_db.add_repo_async(_program("async-p1")),
                )
                assert sync_db.get("async-p0") is not None
                assert sync_db.get("async-p1") is not None
            finally:
